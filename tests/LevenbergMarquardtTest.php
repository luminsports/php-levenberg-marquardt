 ...
 */
function contrivedProblems(): array
{
    $problems = [
        [
            'name'              => 'bennet5([2, 3, 5])',
...
function realWorldProblems(): array
{
    $problems = [
        [
            'name'     => 'fourParamEq',
...
it('fits each contrived problem', function ($name, $curveFunction, $n, $xStart, $xEnd, $problemParameters, $options, $decimalsForErrorValues = 2, $decimalsForParameterValues = 3) {
...
})->with(realWorldProblems())->group('realWorld');

it('returns the solution with the lowest error', function () {
    $xCoords = [
...
it('has zero error for an exact fit', function () {
    $model = createLinearModel(1, 1, 10);

    $this->assertEquals(0, $model->calculateError([1, 1]), 0.001);
})->group('other');

it('predicts the next point on a linear model', function () {
    $model = createLinearModel(1, 1, 10);
...
it('matches error to the sum of differences between the model and data', function () {
    $model = createLinearModel(1, 1, 10);

    $this->assertEquals(0, $model->calculateError([1, 1]), 0.001);
})->group('other');

it('does not recalculate the curve when parameters are unchanged', function () {
    $model = createLinearModel(1, 1, 10);

    $this->assertSame($model->getCurve(), $model->getCurve());
})->group('other');

it('recalculates the curve when parameters change', function () {
    $model = createLinearModel(1, 1, 10);
    $curve = $model->getCurve();
...
it('returns initial values when the function evaluates to NAN after starting', function () {
...
function createLinearModel(float $slope, float $intercept, int $numberOfPoints): LevenbergMarquardt
{
